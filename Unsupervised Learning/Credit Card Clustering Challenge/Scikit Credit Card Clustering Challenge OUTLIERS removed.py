...
# BELOW I TOP OF THE OUTLIERS BASED UPON A NOTEBOOK AT
# https://www.kaggle.com/sabanasimbutt/clustering-visualization-of-clusters-using-pca/comments,
# BUT DIFFERENTLY, AT END IS HER CODE

for c in credit:
    Range = c + '_RANGE'
    credit[Range] = credit[c]

## I REALLY WONDER IF THIS UNDERNEATH IS ANYWHERE CLOSE TO A CORRECT WAY TO DEAL WITH OUTLIERS
...
for c in columns:
    Range = c + '_RANGE'
    #credit[Range] = 0
    credit.loc[((credit[c] > 10000)), Range] = 10000
...
for c in columns:
    Range = c + '_RANGE'
    #credit[Range] = 0
    credit.loc[((credit[c] > 0.9) & (credit[c] <= 1.0)), Range] = 10
...
for c in columns:
    Range = c + '_RANGE'
    #credit[Range] = credit[c]
    credit.loc[((credit[c] > 100)), Range] = 8

# Drop original columns
credit.drop(['BALANCE', 'BALANCE_FREQUENCY', 'PURCHASES',
...
# Silhouette scores might be best for k=6 but still very low
"""


# Call KMeans and calculate silhouette score, can't calculate adjsuetd rand 'cause I don't have an actual y)
kmeans = KMeans(n_clusters=6, init='k-means++',n_init=10, max_iter=300, random_state=123 )
